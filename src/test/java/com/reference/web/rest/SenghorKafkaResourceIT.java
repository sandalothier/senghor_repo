...
import com.reference.config.KafkaProperties;
...
class SenghorKafkaResourceIT {
    ...
    void setup() {
        ...
        SenghorKafkaResource kafkaResource = new SenghorKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/senghor-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/senghor-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
